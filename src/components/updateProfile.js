...
const UpdateProfileComponent = () => {
    ...
    const clickButton = () => {
        // console.log(statevariable);         //userList.push(statevariable);
    ...
    const ClicktoEdit = (index) => {
        console.log(index);
        setShowEdit(index.value);
        updateStatefunction(index.value)
    }
                    ...
                    <tbody>
                        {
                            userList.map((value, index) => {
                                return (
                                    <tr className={value.isStriked ? "stikeout" : "noStrike"} key={index}>
                                        <td>{value.id}</td>
                                        ...
                                        <td>
                                            <button onClick={() => ClicktoDelete(index)}>Delete</button>/{(index === showEdit) ? <button
                                                onClick={() => ClicktoEdit(index)}
                                                key={index}>Edit</button> :
                                                <button onClick={() => saveEditedData(index)}>Save</button>}
                                        </td>
                                    </tr>
                                )
                            })
